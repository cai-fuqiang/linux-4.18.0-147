...
static enum oom_status mem_cgroup_oom(struct mem_cgroup *memcg, gfp_t mask, int order)
{
    ...
    printk("the proc(%s) enter the mem_cgroup oom\n", _task->comm);
	...
	if (memcg->oom_kill_disable) {
		if (!current->in_user_fault) {
            printk("the proc(%s) exit the mem_cgroup_oom SKIPPED\n", _task->comm);
			return OOM_SKIPPED;
        }
		css_get(&memcg->css);
        ...
        printk("the proc(%s) exit the mem_cgroup_oom ASYNC\n", _task->comm);
    ...
    printk("the proc(%s) into the mem_cgroup_out_of_memory\n", _task->comm);
	if (mem_cgroup_out_of_memory(memcg, mask, order)) {
        printk("the proc(%s) exit the mem_cgroup_oom OOM_SUCCESS\n", _task->comm);
		ret = OOM_SUCCESS;
    } else {
        printk("the proc(%s) exit the mem_cgroup_oom OOM_FAILED\n", _task->comm);
		ret = OOM_FAILED;
    }
